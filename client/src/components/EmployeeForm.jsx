...
import { DEPARTMENTS } from "../assets/assets" // Fixed file path string here
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm text-slate-700">
    <div>
     ...
     <div>
      ...
      <input name="allowances" type="number"  min="0" step="0.01" required defaultValue={initialData?.allowances || 0} />
    </div>
     <div>
      <label className="block mb-2">Deductions</label>
      <input name="deductions" type="number"  min="0" step="0.01" required defaultValue={initialData?.deductions || 0} />
    </div>
...
<div className="card p-5 sm:p-6">
  <h3 className="text font-medium text-slate-900 mb-6 pb-4 border-b border-slate-100">Account Setup</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm text-slate-700">
    <div className="sm:col-span-2">
...
<div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
  <button
    type="button"
    className="btn-secondary transform transition-all duration-300 hover:scale-105 hover:-translate-y-0.5 active:scale-95"
  ...
  <button
    type="submit"
    disabled={loading}
    className={`btn-primary flex items-center justify-center transform transition-all duration-300
      ${
        loading
          ? "cursor-not-allowed opacity-80"
          : "hover:scale-105 hover:-translate-y-0.5 hover:shadow-lg active:scale-95"
      }`}
  >
    {loading && (
