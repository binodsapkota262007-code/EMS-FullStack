...
if(data.firstName) setUserName(`${data.firstName} ${data.lastName || ""}`.trim());
    ...
    const sidebarContent = (
        <>
         ...
         {/* Section labels */}
<div className="px-5 pt-5 pb-2 animate-fade-in">
  <p className="
...
<div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
  {loading ? (
    <div className="px-3 py-3 flex items-center gap-2 text-slate-500 animate-pulse">
      ...
      return (
        <Link
          ...
          className={`
            ...
            px-3 py-2.5 rounded-md text-[13px] font-medium
        ...
        >
          {/* Active Indicator */}
          {isActive && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500 animate-pulse" />
          )}
          ...
          {/* Text */}
          <span className="flex-1 transition-all duration-300 group-hover:translate-x-0.5">
            {item.name}
          </span>

          {/* Arrow */}
          {isActive && (
            <ChevronRightIcon className="w-3.5 h-3.5 text-indigo-500/60 animate-pulse" />
          )}
        </Link>
      );
    })
