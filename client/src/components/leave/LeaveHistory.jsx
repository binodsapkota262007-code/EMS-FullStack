...
<tbody>
  ...
  {format(new Date(leave.startDate),"MM dd")} -  {format(new Date(leave.endDate),"MM dd,yyyy")}
</td>
...
<button onClick={() => handleStatusUpdate(leave.id || leave._id, "APPROVED")} disabled={!!processing} className='p-1.5 rounded-md bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors'>

                    {processing === (leave._id || leave.id) ? <Loader2 className="w-4 h-4 animate-spin"/> : <Check className="w-4 h-4"/>}
                </button>

                <button onClick={() => handleStatusUpdate(leave.id || leave._id, "REJECTED")} disabled={!!processing}  className='p-1.5 rounded-md bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors'>
