...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    <p style="font-size: 16px;">EMS</p>
      ...
      // After 1 more hour (10 hours total), mark attendance as checked out with status "LATE"
  ...
  async ({ event, step }) => {
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `<div style="max-width: 600px;">
    <h2>Hi Admin, 👋</h2>
    <p style="font-size: 16px;">You have a pending leave application in ${employee.department}, starting:</p>
...
// Cron: Check attendance at 09:00 AM NPT and email absent employees
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{ cron: "TZ=Asia/Kathmandu 0 10 * * *" }] },
  async ({ step }) => {
    // Step 1: Get today's date range (NPT)
